...
int main() {
    ...
    for (int i = 0; i < amount_of_books_in_library; i++) {
        ...
        books[i] = new Book(arr[i].c_str(), arr1[i].c_str(), arr2[i], arr3[i]); // Передаю в функцию 2 константные ссылки и 2 int'a.Cоздание объектов Book с изменением типов с string на const char*, посредством метода .c_str()     
        cout << endl;
    }
